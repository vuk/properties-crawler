...
- **Implemented today**: a **Node crawler** (`crawler/`) walks links with the `[crawler](https://www.npmjs.com/package/crawler)` package and Cheerio-loaded pages; **site-specific adapters** map HTML to a common `Property` shape and write to **PostgreSQL**. A **standalone Express API** (`backend/`) serves `GET /properties/`, **auth** (`POST /auth/register`, `POST /auth/login`, `GET /auth/me` with `Authorization: Bearer`), **favorites** (authenticated `GET /favorites/ids`, `GET /favorites`, `POST /favorites/:propertyId`, `DELETE /favorites/:propertyId`), and `GET /health` from the same database. The **frontend** (`frontend/`) lists properties, **login / registration** (`/login`, `/register`), and **profile / saved ads** (`/profile`) backed by those routes (Vite dev proxies `/api` → backend).
...
| `backend/`                                 | Express app: entry `src/server.ts`, shared `src/pool.ts`, **esbuild** bundle to `dist/server.js` (`esbuild.config.mjs`). Listing in `src/functions/get-properties.ts` (`getPropertiesResponse`); auth in `src/auth.ts`; favorites in `src/favorites.ts`. |
