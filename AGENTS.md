...
1. **Startup**: `index.ts` instantiates every adapter from `adapter.enum.ts`, connects `Database`, creates a `Crawler` with configurable concurrency (`CRAWLER_MAX_CONNECTIONS`, default **10**; optional `CRAWLER_RATE_LIMIT_MS` — note: in node-crawler, `rateLimit > 0` forces a single in-flight request). URLs to **4zida.rs** and **kupujemprodajem.com** each use a separate Bottleneck limiter (`4zida.rs`, `kupujemprodajem.com`) at **~10 requests/minute** (6s minimum between *starting* fetches, one in flight per host) to reduce **429** responses; other sites use the default limiter.
2. **Seeding**: `initiateCrawl` queues each adapter’s `baseUrl` and `seedUrl` entries (via `queueCrawlUrl`, which attaches those limiters when the host matches).
